...
ipcMain.on("SINGLE-PERSON-CALL", () => {
  mainWindow.setContentSize(WIDTH_DEFAULT * 3, HEIGHT_DEFAULT * 2, true);
});
ipcMain.on("EXIT", () => {
  app.exit();
});
  ...
  switch (obj.type) {
    ...
    case "SHARE-CONTROL-DROP-DOWN":
      if (obj.cameraDropdown) {
        ...
        mainWindow.setSize(550, 90, true);
      }
      break;
    case "SWITCH-TO-SHAREE-SCREEN":
      ...
      let y = -20 + display.bounds.y;
      console.log("x y", x, y);
      // Mark - jiggle happens here
      mainWindow.setPosition(x, y, true);
      mainWindow.hide();
      mainWindow.setSize(550, 85, true);
      mainWindow.setHasShadow(false);
      mainWindow.show();
...
function createWindow() {
  ...
  ipcMain.on("AUTO-UPDATE", function (event) {
    ...
    try {
      ...
      log.warn("Error", "Failed to install updates", e);
    }
  });
